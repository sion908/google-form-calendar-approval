...
sequenceDiagram
    ...
    Googleスプレッドシート->>GoogleAppsScript: 回答記録をトリガーにonFormSubmit発火
...
本プロジェクトのGoogle Apps Scriptは、以下のファイルに分割されています。

*   `main.gs`:
    *   フォーム送信時のトリガー関数 (`onFormSubmit`) を含むメインスクリプトです。
    *   Webアプリケーションとしてデプロイされた際のリクエスト処理 (`doGet`, `doPost`) を担当します。
*   `approval.gs`:
    *   承認・否認処理のロジックを定義します。
    *   スプレッドシートのステータス更新など、承認フローの中核を担います。
*   `calendar.gs`:
    *   Googleカレンダーへのイベント作成、更新、削除など、カレンダー操作に関する関数を定義します。
*   `email.gs`:
    *   承認依頼メール、承認完了メール、否認メールなど、各種メール送信に関する関数を定義します。
*   `properties.gs`:
    *   スクリプトプロパティの設定、取得、削除など、プロパティ管理に関する関数を定義します。
    *   スプレッドシートIDやカレンダーIDなど、環境固有の情報を安全に管理します。
*   `constants.gs`:
    *   スプレッドシートの列番号や固定文字列など、変更頻度が低い定数を定義します。
